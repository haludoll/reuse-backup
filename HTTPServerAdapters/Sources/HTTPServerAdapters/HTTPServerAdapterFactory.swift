...
public enum HTTPServerAdapterFactory {
    ...
    public static func createServer(port: UInt16) -> HTTPServerAdapterProtocol {
        if #available(iOS 17.0, *) {
            ...
            // iOS 15-16: HummingBird v1.x は利用できないため、v2を使用（警告付き）
            print("Warning: iOS 15-16 detected. HummingBird v1.x is not available with v2 package. Using HummingBirdV2Adapter.")
            if #available(iOS 17.0, *) {
                return HummingBirdV2Adapter(port: port)
            } else {
